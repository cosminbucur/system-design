...
Once complete, both sides have an open, reliable byte stream — but everything sent over it is plaintext. Anyone able to observe the wire can read it. TCP solves *delivery*, not *privacy*.
...
Asymmetric encryption (public/private key) is only used to bootstrap trust and exchange the session key; the actual bulk data is encrypted with the resulting *symmetric* session key, because symmetric encryption is far faster for volume.
